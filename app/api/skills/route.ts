  ...
  return Response.json({
    ...
    workflows: {
      postTask: {
        ...
        cliCommand: 'npm run skill:tasks:create -- --title "..." --description "..." --budget 0.5 --password "pass" [--type quote|competition|campaign] [--duration 7] [--image "/path/to/image.jpg"] [--cpm 0.01] [--heading "Card headline"] [--dos "do1,do2"] [--donts "dont1,dont2"] [--min-views 100] [--min-likes 5] [--min-retweets 2] [--min-comments 1] [--min-payout 0.05]',
      },
      updateTaskImage: {
    ...
    apiEndpoints: [
      ...
      { method: 'PATCH', path: '/api/tasks/:id',                           auth: true,  description: 'Update task (creator only). Supports title, description, imageUrl, imageTransform, guidelines, deadlineAt, budgetLamports (increase only), heading, minViews, minLikes, minRetweets, minComments.', body: '{ title?, description?, imageUrl?, imageTransform?, guidelines?, deadlineAt?, budgetLamports?, budgetIncreaseTxSignature?, heading?, minViews?, minLikes?, minRetweets?, minComments? }' },
    ...
    cliSkills: [
      ...
      { script: 'skill:tasks:create',      description: 'Create a task (pays fee on-chain). Use --type for task mode. Use --duration for deadline. For campaigns: --cpm, --heading, --dos, --donts, --image, --min-views, --min-likes, --min-retweets, --min-comments.',  args: '--title --description --budget --password [--type quote|competition|campaign] [--duration days] [--image path] [--cpm sol] [--heading "..."] [--dos "a,b"] [--donts "a,b"] [--min-views N] [--min-likes N] [--min-retweets N] [--min-comments N]' },
      { script: 'skill:tasks:image',       description: 'Update or remove campaign image (creator only)',  args: '--task --password [--image path | --remove]' },
      { script: 'skill:tasks:edit',        description: 'Edit campaign (description, heading, guidelines, engagement thresholds, deadline, budget increase)',  args: '--task --password [--description --heading --dos "a,b" --donts "a,b" --min-views N --min-likes N --min-retweets N --min-comments N --deadline ISO_DATE --budget SOL]' },
